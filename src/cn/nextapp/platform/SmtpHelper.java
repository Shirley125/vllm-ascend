...
public class SmtpHelper {
	...
	public static String getAdministrator() {
		return smtp_props.getProperty("smtp.administrator");
	}

	public static String getEditor() {
		return smtp_props.getProperty("smtp.editor");
	}
	 ...
	 * @throws Exception 
	 */
	public static void sendBuildEndNotifyMail(final User user, final App app, final Version ver, boolean success) throws Exception {
		String title = ResourceUtils.getString("ui", success?"build_success":"build_failed", app.getName());
		String tpl = "/WEB-INF/pages/mail/"+(success?"build_success.html":"build_failed.html");
		String html = VelocityHelper.execute(tpl, new HashMap<String, Object>(){{
			put("user",user);
			put("app", app);
			put("ver", ver);
		}});
		if(FormatTool.is_email(user.getEmail()))
			send(user.getEmail(), title, html);
	}

	public static void sendToAdministrator(String title, String content) throws EmailException {
		String email = other_props.getProperty("administrator");
